...
for candidate in range(len(candidates)):
    log_bfs_one_qpo = []
    try:
        res_no_qpo = bilby.result.read_in_result(f"sliding_window_{band}_candidates/no_qpo/{candidate}_result.json")
        res_one_qpo = bilby.result.read_in_result(f"sliding_window_{band}_candidates/one_qpo/{candidate}_result.json")
...
np.savetxt(f'sliding_window_{band}_candidates/log_bfs_one_qpo_{candidate}', np.array(log_bfs_one_qpo))
