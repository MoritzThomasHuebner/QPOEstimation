...
outdir_white_noise_periodogram = f'injection/general_qpo_injection/white_noise_recovery/whittle/results/'
...
for start_time, end_time, duration in zip(start_times, end_times, durations):
    ...
    try:
        res_noise = bilby.result.read_in_result(outdir=outdir_white_noise_periodogram, label=label_red_noise)
    ...
    psd_array_noise_max_like = white_noise_max_like * np.ones(len(frequencies))
    psd_array_qpo_max_like = QPOEstimation.model.psd.lorentzian(
        frequencies=frequencies, amplitude=amplitude_max_like, width=width_max_like,
        central_frequency=central_frequency_max_like)
    psd_array_signal_max_like = psd_array_noise_max_like + psd_array_qpo_max_like
...
plt.fill_between(extension_factors, snrs_max_like_quantiles[:, 0], snrs_max_like_quantiles[:, 1], color='#ff7f0e',
                 alpha=0.3)
plt.xlabel(r'Extension factor $x$')
plt.ylabel(r'SNR')
plt.legend()
