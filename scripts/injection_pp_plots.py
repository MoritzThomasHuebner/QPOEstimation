    ...
    for i in range(0, 100):
        try:
            ...
            if likelihood_model == 'gaussian_process_windowed':
                res = bilby.result.read_in_result(f'injection_{band_minimum}_{band_maximum}Hz_normal_{injection_mode}/{injection_mode}/results/{i}_gaussian_process_windowed_result.json')
            else:
                res = bilby.result.read_in_result(f'injection_{band_minimum}_{band_maximum}Hz_normal_{injection_mode}/{injection_mode}/results/{i}_gaussian_process_result.json')
