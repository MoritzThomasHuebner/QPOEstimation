...
for j in range(9):
    ...
    for i in range(31):
        try:
            res_no_qpo = bilby.result.read_in_result(f"sliding_window/period_{j}/two_qpo/{i}_no_qpo_result.json")
            res_qpo = bilby.result.read_in_result(f"sliding_window/period_{j}/two_qpo/{i}_qpo_result.json")
